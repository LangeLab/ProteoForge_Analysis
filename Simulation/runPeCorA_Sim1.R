...
data_id <- args[1]
# Get the experiment type (perturbation)
perturb <- args[2]

cat("Running PeCorA on the simulated data in", data_id, "folder with", perturb, "perturbation.\n")
dataVersions <- c("complete", "imputed")
for (version in dataVersions){
    cat("\nWorking on", version, "\n")
    input_data_path <- paste0('./data/exp/', data_id, "/", version, "_", perturb, "_InputData.feather")
    ...
    output_path <- paste0('./data/exp/', data_id, "/", version, "_", perturb,  "_PeCorA_ModelResults.feather")
