...
cat("Running COPF on the simulated data in", data_id, "folder\n")
missRatePeps <- c(0, 0.25, 0.5, 0.75)
missRatePros <- c(0, 0.25, 0.5, 0.75)
for (i in missRatePros){
    for (j in missRatePeps){
        cat("Running COPF on the simulated data with rates", i, "of missing proteins and", j, "of missing peptides\n")
        # Read the input data (feather format) with path
        input_data_path <- paste0('./data/exp/', data_id, "/imputed_Pro", i, "_Pep", j, "_InputData.feather")
        ...
        output_path <- paste0('./data/exp/', data_id, "/imputed_Pro", i, "_Pep", j, "_COPF_ModelResults.feather")
