# ==============================================================================
# run_r_methods_mnar.R
#
# FDR calibration for COPF and PeCorA under null simulation WITH MNAR
# missingness. Companion to the MNARCalibration notebook.
#
# Prerequisite: run generate_mnar_null_data.py first to create
#     Revisions/outputs/simulation/mnar_fdr_calibration/null_input/
#         mnar_null_run_{k}_InputData.feather
#
# Output CSVs:
#     Revisions/outputs/simulation/mnar_fdr_calibration/rmethod_id_fpr_K<K>.csv
#     Revisions/outputs/simulation/mnar_fdr_calibration/rmethod_grp_fpr_K<K>.csv
#
# Usage (from project root):
#     PF_SIM_K=50 Rscript Revisions/logic/r/simulation/fdr_calibration/run_r_methods_mnar.R
...
output_dir <- file.path(project_root, "Revisions", "outputs", "simulation", "mnar_fdr_calibration")
...
cat("================================================================================\n")
cat("     MNAR FDR CALIBRATION — R Methods (COPF & PeCorA)   K =", K, "\n")
cat("================================================================================\n")
cat("PF_SIM_K:", K, "\n")
...
for (k in 0:(K - 1)) {
    ...
    input_file  <- file.path(input_dir,  sprintf("mnar_null_run_%d_InputData.feather", k))
    copf_cache  <- file.path(output_dir, sprintf("copf_mnar_null_run%02d.feather",     k))
    pec_cache   <- file.path(output_dir, sprintf("pecora_mnar_null_run%02d.feather",   k))
    ...
    if (!file.exists(copf_cache)) {
        ...
        traces <- CCprofiler::clusterPeptides(
            ...
            plot   = FALSE,
            PDF    = FALSE,
            name   = sprintf("COPF_mnar_null_run%02d", k)
        )
    ...
    # Use RAW p-values (pvalue, not adj_pval) for fair comparison
    pec_df    <- as.data.table(arrow::read_feather(pec_cache))
    n_pep_pec <- nrow(pec_df)

    for (a in alpha_grid) {
        n_rej <- sum(pec_df$pvalue < a, na.rm = TRUE)
...
cat("================================================================================\n")
cat("Done. Load the CSVs in the MNARCalibration notebook to add R-method curves.\n")
cat("================================================================================\n")
